...
class HelloNode:
    ...
    def get_storage(self, name):
        new_storage = self.storage_generator(f"{self.node_id}-{name}")
        self.storages[name] = new_storage
        return new_storage

    def get_compute(self, name):
        new_compute = self.compute_generator(f"{self.node_id}-{name}")
    ...
    def run_monitor(self, event_delta, time_delta):
        ...
        for compute in self.computes:
            ...
            self.monitor.add_value(f"compute-{compute}-resource", self.computes[compute].resource_utilization(event_delta))
